"""
Testskript für den StoryProcessor.
Dieses Skript testet die Funktionalität des StoryProcessor nach den Korrekturen.
"""
...
from src.core.models.story import StoryProcessorInput, StoryProcessorOutput, StoryData, StoryProcessingResult
...
def create_test_topic_and_target_group(processor):
    """Erstellt ein Testtopic und eine Testzielgruppe, falls sie nicht existieren."""
    # Testtopic: Gemeinschaftsbildung
    community_topic = {
        ...
        "description": {
            "de": "Methoden und Praktiken zur Förderung des gemeinschaftlichen Zusammenhalts",
            "en": "Methods and practices to foster community cohesion"
        },
        "keywords": ["community", "togetherness", "social", "cohesion"],
    ...
    general_target = {
        ...
        "description": {
            "de": "Allgemeine Zielgruppe ohne spezifische Ausrichtung",
            "en": "General target group without specific focus"
        },
        "status": "active"
    }
    
    try:
        # Prüfen, ob das Topic bereits existiert
        existing_topic = processor.story_repository.get_topic_by_id("gemeinschaftsbildung")
        if not existing_topic:
            print("Erstelle Topic 'Gemeinschaftsbildung'...")
            processor.story_repository.create_topic(community_topic)
        else:
            print("Topic 'Gemeinschaftsbildung' existiert bereits.")
        
        # Prüfen, ob die Zielgruppe bereits existiert
        existing_target = processor.story_repository.get_target_group_by_id("general")
        if not existing_target:
            print("Erstelle Zielgruppe 'general'...")
            processor.story_repository.create_target_group(general_target)
        else:
...
def get_test_session_ids(processor):
    """Ruft einige Test-Session-IDs ab."""
    print("Lade Session-IDs für das Topic 'Gemeinschaftsbildung'...")
    
    # Rufe Sessions mit dem Filter ab
    cursor = processor.story_repository.db.session_cache.find({"data.topic": "Gemeinschaftsbildung"})
    sessions = list(cursor)
    
    # Extrahiere die Session-IDs
    session_ids = [str(session["_id"]) for session in sessions[:10]]  # Verwende nur die ersten 10 Sessions
    
    print(f"{len(session_ids)} Session-IDs gefunden: {session_ids}")
    return session_ids
...
async def create_direct_story(processor, topic_id, event, target_group, languages, session_data, transformer_processor=None):
    ...
    target_group = story_repo.get_target_group_by_id(target_group)
    
    if not topic or not target_group:
        print(f"Fehler: Thema {topic_id} oder Zielgruppe {target_group} nicht gefunden")
        return None
    
    # Verzeichnisstruktur erstellen
    base_dir = f"stories/{event}_{target_group.get('target_id', 'unknown')}/{topic_id}"
        ...
        if transformer_processor and TRANSFORMER_AVAILABLE:
            try:
                ...
                context = {
                    ...
                    "event": event,
                    "target_group": target_group.get("target_id", ""),
...
""")
        ...
        await create_test_topic_and_target_group(processor)
