"""
Test für den VideoProcessor mit MongoDB-Caching.

Dieses Skript testet die MongoDB-Caching-Funktionalität des VideoProcessors.
"""
...
from src.core.mongodb.connection import get_mongodb_database, get_mongodb_client
from src.core.resource_tracking import ResourceCalculator
from src.processors.video_processor import VideoProcessor
from src.core.models.video import VideoSource, VideoProcessingResult, VideoMetadata
from pymongo import IndexModel, ASCENDING

# Konfiguriere Logging direkt
logging.basicConfig(level=logging.INFO)
logger = get_logger(process_id="test-video-processor")
...
def test_direct_mongodb_cache():
    ...
    ohne die Videoverarbeitung durchzuführen.
    """
    print("\n===== Direkter MongoDB-Cache-Test =====")
    
    # MongoDB-Verbindung herstellen
    db = get_mongodb_database()
    collection = db["video_cache"]
    ...
    try:
        ...
        if found_data:
            ...
            print(f"  Source URL: {found_data.get('source_url')}")
            print(f"  Created At: {found_data.get('created_at')}")
            
            # Anzahl der Einträge in der Collection ausgeben
            count = collection.count_documents({})
            print(f"Anzahl der Einträge in der video_cache Collection: {count}")
...
def test_video_processor_caching():
    """
    Testet den VideoProcessor mit MongoDB-Caching.
    
    Der Test führt folgende Schritte aus:
    1. MongoDB-Cache initialisieren
    2. VideoProcessor mit aktiviertem Caching erstellen
    3. Prüfe, ob der VideoProcessor korrekt initialisiert wurde
    4. Teste die Verarbeitung einer lokalen Videodatei mit Caching
    """
    print("Starte VideoProcessor-Caching-Test")
    ...
    try:
        ...
        # VideoProcessor mit aktiviertem Caching erstellen
        print("Erstelle VideoProcessor...")
        processor = VideoProcessor(resource_calculator, process_id)
        print("VideoProcessor erstellt.")
        
        # Prüfe, ob der VideoProcessor korrekt initialisiert wurde
        if not hasattr(processor, 'cache_collection_name'):
            print("FEHLER: VideoProcessor hat kein 'cache_collection_name'-Attribut")
            return
            
        print(f"VideoProcessor Cache-Collection: {processor.cache_collection_name}")
        
        # Prüfe, ob der VideoProcessor vom CacheableProcessor erbt
        print(f"VideoProcessor erbt von: {processor.__class__.__mro__}")
        
        # Teste die Erstellung eines Cache-Schlüssels
        test_url = "https://example.com/video.mp4"
        # Verwende die öffentliche Methode generate_cache_key statt der geschützten Methode
        cache_key = processor.generate_cache_key(test_url)
        print(f"Test Cache-Schlüssel für URL '{test_url}': {cache_key}")
        ...
        test_file_path = Path("tests/samples/hello.mp4")
        if test_file_path.exists():
            print(f"Teste Verarbeitung der lokalen Datei: {test_file_path}")
            
            # Erstelle VideoSource-Objekt
            with open(test_file_path, "rb") as f:
                file_content = f.read()
            
            # VideoSource mit Datei-Inhalt und Dateinamen erstellen
            ...
            first_response = asyncio.run(processor.process(
                source=video_source,  # VideoSource-Objekt statt Stringpfad
                target_language="de",
                use_cache=True
            ))
